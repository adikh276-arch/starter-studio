import { useState, useEffect, useCallback } from 'react';
import { fetchUserGoals, fetchGoalEntries, addGoal as addGoalAction, deleteGoal as deleteGoalAction, saveGoalEntry } from '@/app/actions/goalActions';
import { getUserId } from '../lib/auth';

export interface DayEntry {
  id?: string;
  goal_id: string;
  date: string;
  drive: number;
  energy: number;
  focus: number;
  clarity: number;
  tookAction: boolean;
  impactLevel: number;
  actionNote: string;
  blocker: string;
}

export interface Goal {
  id: string;
  user_id: string;
  name: string;
  category: string;
  createdAt: string;
}

interface GoalStore {
  goals: Goal[];
  entries: Record<string, DayEntry[]>; // goalId -> entries
  loading: boolean;
}

export const defaultEntry = (goalId: string, date: string): DayEntry => ({
  goal_id: goalId,
  date,
  drive: 5,
  energy: 5,
  focus: 5,
  clarity: 5,
  tookAction: false,
  impactLevel: 5,
  actionNote: '',
  blocker: '',
});

export function useGoalStore() {
  const [store, setStore] = useState<GoalStore>({ goals: [], entries: {}, loading: true });
  const userId = getUserId();

  const fetchAll = useCallback(async () => {
    if (!userId) return; // Wait for AuthProvider
    setStore(prev => ({ ...prev, loading: true }));
    try {
      const goalsResult = await fetchUserGoals(userId);
      const rawGoals = goalsResult.success && goalsResult.data ? goalsResult.data : [];
      const goals: Goal[] = rawGoals.map((g: any) => ({
        id: g.id,
        user_id: g.user_id,
        name: g.name,
        category: g.category || 'learning',
        createdAt: g.created_at
      }));
      
      const entriesResult = await fetchGoalEntries(userId);
      const entriesRaw = entriesResult.success && entriesResult.data ? entriesResult.data : [];

      const entries: Record<string, DayEntry[]> = {};
      entriesRaw.forEach((e: any) => {
        if (!entries[e.goal_id]) entries[e.goal_id] = [];
        entries[e.goal_id].push({
          id: e.id,
          goal_id: e.goal_id,
          date: formatDateKey(e.date),
          drive: Number(e.drive),
          energy: Number(e.energy),
          focus: Number(e.focus),
          clarity: Number(e.clarity),
          tookAction: e.took_action,
          impactLevel: Number(e.impact_level),
          actionNote: e.action_note || '',
          blocker: e.blocker || '',
        });
      });

      setStore({ goals, entries, loading: false });
    } catch (err) {
      console.error('Failed to fetch data:', err);
      setStore(prev => ({ ...prev, loading: false }));
    }
  }, [userId]);

  useEffect(() => {
    fetchAll();
  }, [fetchAll]);

  const addGoal = useCallback(async (name: string, category: string = 'learning') => {
    try {
      const result = await addGoalAction({ userId: userId!, name, category });
      if (!result.success || !result.data) throw new Error('Failed to add goal');
      const rawGoal = result.data;
      const newGoal: Goal = {
        id: rawGoal.id,
        user_id: rawGoal.user_id,
        name: rawGoal.name,
        category: rawGoal.category,
        createdAt: rawGoal.created_at
      };
      setStore(prev => ({
        ...prev,
        goals: [newGoal, ...prev.goals],
        entries: { ...prev.entries, [newGoal.id]: [] },
      }));
      return newGoal;
    } catch (err) {
      console.error('Failed to add goal:', err);
      return null;
    }
  }, [userId]);

  const deleteGoal = useCallback(async (goalId: string) => {
    try {
      await deleteGoalAction(goalId);
      setStore(prev => {
        const { [goalId]: _, ...rest } = prev.entries;
        return {
          ...prev,
          goals: prev.goals.filter(g => g.id !== goalId),
          entries: rest,
        };
      });
    } catch (err) {
      console.error('Failed to delete goal:', err);
    }
  }, []);

  return { store, setStore, addGoal, deleteGoal, refresh: fetchAll };
}

export function useGoalData(goalId: string, store: GoalStore, setStore: React.Dispatch<React.SetStateAction<GoalStore>>) {
  const todayKey = formatDateKey(new Date());
  const entries = store.entries[goalId] || [];

  const [todayEntry, setTodayEntry] = useState<DayEntry>(
    entries.find(e => e.date === todayKey) || defaultEntry(goalId, todayKey)
  );

  useEffect(() => {
    const fresh = entries.find(e => e.date === todayKey);
    if (fresh) {
      setTodayEntry(fresh);
    } else {
      setTodayEntry(defaultEntry(goalId, todayKey));
    }
  }, [entries, todayKey, goalId]);

  const updateToday = useCallback((updates: Partial<DayEntry>) => {
    setTodayEntry(prev => ({ ...prev, ...updates }));
  }, []);

  const save = useCallback(async () => {
    try {
      const result = await saveGoalEntry({
        goalId,
        dateKey: todayKey,
        drive: todayEntry.drive,
        energy: todayEntry.energy,
        focus: todayEntry.focus,
        clarity: todayEntry.clarity,
        tookAction: todayEntry.tookAction,
        impactLevel: todayEntry.impactLevel,
        actionNote: todayEntry.actionNote,
        blocker: todayEntry.blocker
      });
      if (!result.success || !result.data) throw new Error('Failed to save entry');
      const saved = result.data;

      const updatedEntry: DayEntry = {
        id: saved.id,
        goal_id: saved.goal_id,
        date: formatDateKey(saved.date),
        drive: Number(saved.drive),
        energy: Number(saved.energy),
        focus: Number(saved.focus),
        clarity: Number(saved.clarity),
        tookAction: saved.took_action,
        impactLevel: Number(saved.impact_level),
        actionNote: saved.action_note || '',
        blocker: saved.blocker || '',
      };

      setStore(prev => {
        const goalEntries = prev.entries[goalId] || [];
        const idx = goalEntries.findIndex(e => e.date === todayKey);
        const next = idx >= 0
          ? [...goalEntries.slice(0, idx), updatedEntry, ...goalEntries.slice(idx + 1)]
          : [...goalEntries, updatedEntry];
        return { ...prev, entries: { ...prev.entries, [goalId]: next } };
      });
    } catch (err) {
      console.error('Failed to save entry:', err);
    }
  }, [goalId, todayKey, todayEntry, setStore]);

  const last7Days = (entries || [])
    .filter(e => {
      if (!e.date) return false;
      const d = new Date(e.date + 'T00:00:00'); // Force local midnight
      const today = new Date();
      today.setHours(23, 59, 59, 999); // End of today
      const weekAgo = new Date();
      weekAgo.setDate(weekAgo.getDate() - 7);
      weekAgo.setHours(0, 0, 0, 0); // Start of 7 days ago
      
      return d >= weekAgo && d <= today;
    })
    .sort((a, b) => a.date.localeCompare(b.date));

  const avg7Day = last7Days.length > 0
    ? Math.round((last7Days.reduce((s, e) => s + (Number(e.drive) + Number(e.energy) + Number(e.focus) + Number(e.clarity)) / 4, 0) / last7Days.length) * 10) / 10
    : 0;

  const actionDays = last7Days.filter(e => e.tookAction).length;
  const avgWithAction = last7Days.filter(e => e.tookAction);
  const avgWithoutAction = last7Days.filter(e => !e.tookAction);

  const actionAvg = avgWithAction.length > 0
    ? avgWithAction.reduce((s, e) => s + (Number(e.drive) + Number(e.energy) + Number(e.focus) + Number(e.clarity)) / 4, 0) / avgWithAction.length
    : 0;
  const noActionAvg = avgWithoutAction.length > 0
    ? avgWithoutAction.reduce((s, e) => s + (Number(e.drive) + Number(e.energy) + Number(e.focus) + Number(e.clarity)) / 4, 0) / avgWithoutAction.length
    : 0;

  const multiplier = noActionAvg > 0 ? (actionAvg / noActionAvg) : 0;

  return {
    todayEntry,
    updateToday,
    save,
    entries,
    last7Days,
    avg7Day,
    actionDays,
    multiplier,
  };
}

function formatDateKey(date: string | Date): string {
  const d = new Date(date);
  const year = d.getFullYear();
  const month = String(d.getMonth() + 1).padStart(2, '0');
  const day = String(d.getDate()).padStart(2, '0');
  return `${year}-${month}-${day}`;
}
