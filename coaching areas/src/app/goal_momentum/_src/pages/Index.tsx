...
import { ChevronDown, ChevronLeft, History, ArrowLeft, Crosshair } from 'lucide-react';
...
function GoalTracker({ goal, store, setStore, onBack }: {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onBack}
        className="flex h-10 w-10 items-center justify-center rounded-xl bg-white shadow-sm border border-border hover:bg-muted transition-all"
      ...
      >
        ...
        <div className="pt-6 space-y-4">
          ...
          <ShareModal t={t} 
            isOpen={glowCard} 
            onClose={() => setGlowCard(false)} 
            title={t('share.goal_momentum_title')}
            message={t('share.goal_momentum_msg')}
          />
        </div>
      </motion.div>
      ...
      <div className="w-full">
        <button
          type="button"
          className="glass-card w-full p-4 flex items-center justify-between cursor-pointer hover:bg-slate-50/50 transition-colors"
