...
export function CareTeam() {
  ...
  const [showSwitchExpertModal, setShowSwitchExpertModal] = useState(false);
  const [showSwitchFeedbackModal, setShowSwitchFeedbackModal] = useState(false);
  const [showPreferencesModal, setShowPreferencesModal] = useState(false);
  const [showReportProviderModal, setShowReportProviderModal] = useState(false);
  ...
  const [showSessionsDetailsModal, setShowSessionsDetailsModal] = useState(false);
  ...
  return (
    <div className="flex min-h-screen bg-[#F9FAFB]">
                ...
                <div className="bg-white border-b border-[#E2ECF5] px-4 md:px-6 py-4 flex-shrink-0">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <motion.button
                        onClick={() => router.push(`/provider-profile/${selectedExpert.id}`)}
                    ...
                    <div className="flex items-center gap-2">
                      {selectedExpert.id !== "1" && selectedExpert.id !== "1b" && selectedExpert.id !== "1c" && selectedExpert.id !== "1d" && (
                        <>
                          <motion.button
                            ...
                            <>
                              ...
                              >
                                ...
                                <button
                                  onClick={() => {
                                    setShowOptionsMenu(false);
                                    router.push(`/appointments?provider=${encodeURIComponent(selectedExpert.name)}`);
                                  }}
      ...
      <ScheduleAppointmentModal
        ...
        expertTitle={selectedExpert?.specialty || ""}
        expertImage={selectedExpert?.image || ""}
      />

      {/* Switch Expert Modal */}
      <SwitchExpertModal
        isOpen={showSwitchExpertModal}
        onClose={() => setShowSwitchExpertModal(false)}
        currentExpertRole={selectedExpert?.role || "Expert"}
        onExpertSelect={(expertId) => {
          // Handle expert selection
          setShowSwitchExpertModal(false);
        }}
        onPreferencesClick={() => {
          setShowSwitchExpertModal(false);
      ...
      {/* Switch Feedback Modal */}
      <FeedbackModal
        isOpen={showSwitchFeedbackModal}
        onClose={() => setShowSwitchFeedbackModal(false)}
        onSubmit={() => {
          setShowSwitchFeedbackModal(false);
          setShowSwitchExpertModal(true);
        }}
        expertRole={selectedExpert?.role || "Expert"}
      />
      ...
      <ReportProviderModal
        isOpen={showReportProviderModal}
        onClose={() => setShowReportProviderModal(false)}
        providerName={selectedExpert?.name || ""}
        onSubmit={(reasons, otherReason) => {
          console.log("Report submitted:", { reasons, otherReason, expert: selectedExpert?.name });
      ...
      <BuyPlanModal
        ...
        userType={buyPlanType}
        userName="Rachit"
        serviceName={selectedExpert?.specialty || "Therapy"}
        providerName={selectedExpert?.name || "Julie"}
        monthlyRemaining={0}
        monthlyTotal={4}
        totalRemaining={4}
        totalSessions={48}
        onTypeChange={setBuyPlanType}
        showDevToolbar={showDevToolbar}
        onShowSessionDetails={() => setShowSessionsDetailsModal(true)}
      />

      {/* Sessions Details Modal */}
      <SessionsDetailsModal
        isOpen={showSessionsDetailsModal}
        onClose={() => setShowSessionsDetailsModal(false)}
      />
