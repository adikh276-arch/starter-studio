...
export function PureOOCDPage() {
  ...
  const tools = [
    {
      ...
      label: t("thought_diffusion"),
      icon: Clock,
      bgColor: "#FAF5FF",
      iconColor: "#A855F7",
      url: "/thought_diffusion"
    },
    {
      id: "thought-truth",
      label: t("thought_truth"),
      icon: Lightbulb,
      bgColor: "#EFF6FF",
      iconColor: "#3B82F6",
      url: "/thought_truth"
    },
    {
      ...
      label: t("uncertainty_acceptance_practice"),
      icon: Target,
      bgColor: "#F0FDF4",
      iconColor: "#22C55E",
      url: "/uncertainity_acceptance"
    },
    {
      id: "letter-to-ocd",
      label: t("letter_to_ocd"),
      icon: Mail,
      bgColor: "#FDF2F8",
      iconColor: "#EC4899",
      url: "/letter_to_ocd"
  ...
  const categoryData = wellnessData['pure-o-ocd'] || { tips: [], myths: [], articles: [], stories: [] };
        ...
        <main className="max-w-[1050px] w-full mx-auto px-6 md:px-8 py-8 md:py-12">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-[#020817] tracking-tight">{t("pure_o_ocd")}</h1>
                <p className="text-sm text-[#64748B]">{t("pure_o_ocd_desc")}</p>
              </div>
            </div>
          </motion.div>
                    ...
                    >
                      <div className="px-6 pb-6 space-y-3">
                        {categoryData.tips.map((tip, index) => (
                          ...
                          >
                            <h4 className="text-[15px] font-bold text-[#1E293B] mb-1.5 group-hover:text-[#4F46E5] transition-colors">{index + 1}. {t(`pure_o_ocd_tip_${tip.id}_title`, tip.title)}</h4>
                            <p className="text-[13px] text-[#64748B] leading-relaxed">{t(`pure_o_ocd_tip_${tip.id}_content_0`, tip.content[0])}</p>
                          </button>
                        ))}
                      </div>
                    </motion.div>
                    ...
                    >
                      <div className="px-6 pb-6 space-y-3">
                        {categoryData.myths.map((myth, index) => (
                          ...
                          >
                            <h4 className="text-[15px] font-bold text-[#1E293B] mb-1.5 group-hover:text-[#E11D48] transition-colors">{index + 1}. {t(`pure_o_ocd_myth_${myth.id}_title`, myth.title)}</h4>
                            <p className="text-[13px] text-[#64748B] leading-relaxed">{t(`pure_o_ocd_myth_${myth.id}_content_0`, myth.content[0])}</p>
                          </button>
                        ))}
                      </div>
                    </motion.div>
                    ...
                    >
                      <div className="px-6 pb-6 space-y-3">
                        {categoryData.articles.map((article, index) => (
                          ...
                          >
                            <h4 className="text-[15px] font-bold text-[#1E293B] mb-1.5 group-hover:text-[#2563EB] transition-colors">{index + 1}. {t(`pure_o_ocd_article_${article.id}_title`, article.title)}</h4>
                    ...
                    >
                      <div className="px-6 pb-6 space-y-3">
                        {categoryData.stories.map((story, index) => (
                          ...
                          >
                            <h4 className="text-[15px] font-bold text-[#1E293B] mb-1.5 group-hover:text-[#0D9488] transition-colors">{index + 1}. {t(`pure_o_ocd_story_${story.id}_title`, story.title)}</h4>
                            <p className="text-[13px] text-[#64748B] leading-relaxed">{t(`pure_o_ocd_story_${story.id}_content_0`, story.content[0])}</p>
                          </button>
                        ))}
                      </div>
                    </motion.div>
