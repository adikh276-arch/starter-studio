...
export function ContaminationOCDPage() {
  ...
  const tools = [
    ...
    },
    {
      ...
      label: t("uncertainty_tolerance"),
      icon: Timer,
      bgColor: "#F0F9FF",
      iconColor: "#0369A1",
      url: "/uncertainity_tolerance"
    },
    {
      id: "ritual-cost",
      label: t("ritual_cost_calculator"),
      icon: Wind,
      bgColor: "#FEF2F2",
      iconColor: "#EF4444",
      url: "/ritual_cost"
  ...
  const categoryData = wellnessData['contamination-ocd'] || { tips: [], myths: [], articles: [], stories: [] };
        ...
        <main className="max-w-[1050px] w-full mx-auto px-6 md:px-8 py-8 md:py-12">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-[#020817] tracking-tight">{t("contamination_ocd")}</h1>
                <p className="text-sm text-[#64748B]">{t("contamination_ocd_desc")}</p>
              </div>
            </div>
          </motion.div>
                    ...
                    >
                      <div className="px-6 pb-6 space-y-3">
                        {categoryData.tips.map((tip, index) => (
                          ...
                          >
                            <h4 className="text-[15px] font-bold text-[#1E293B] mb-1.5 group-hover:text-[#4F46E5] transition-colors">{index + 1}. {t(`contamination_ocd_tip_${tip.id}_title`, tip.title)}</h4>
                            <p className="text-[13px] text-[#64748B] leading-relaxed">{t(`contamination_ocd_tip_${tip.id}_content_0`, tip.content[0])}</p>
                          </button>
                        ))}
                      </div>
                    </motion.div>
                    ...
                    >
                      <div className="px-6 pb-6 space-y-3">
                        {categoryData.myths.map((myth, index) => (
                          ...
                          >
                            <h4 className="text-[15px] font-bold text-[#1E293B] mb-1.5 group-hover:text-[#E11D48] transition-colors">{index + 1}. {t(`contamination_ocd_myth_${myth.id}_title`, myth.title)}</h4>
                            <p className="text-[13px] text-[#64748B] leading-relaxed">{t(`contamination_ocd_myth_${myth.id}_content_0`, myth.content[0])}</p>
                          </button>
                        ))}
                      </div>
                    </motion.div>
                    ...
                    >
                      <div className="px-6 pb-6 space-y-3">
                        {categoryData.articles.map((article, index) => (
                          ...
                          >
                            <h4 className="text-[15px] font-bold text-[#1E293B] mb-1.5 group-hover:text-[#2563EB] transition-colors">{index + 1}. {t(`contamination_ocd_article_${article.id}_title`, article.title)}</h4>
                    ...
                    >
                      <div className="px-6 pb-6 space-y-3">
                        {categoryData.stories.map((story, index) => (
                          ...
                          >
                            <h4 className="text-[15px] font-bold text-[#1E293B] mb-1.5 group-hover:text-[#0D9488] transition-colors">{index + 1}. {t(`contamination_ocd_story_${story.id}_title`, story.title)}</h4>
                            <p className="text-[13px] text-[#64748B] leading-relaxed">{t(`contamination_ocd_story_${story.id}_content_0`, story.content[0])}</p>
                          </button>
                        ))}
                      </div>
                    </motion.div>
