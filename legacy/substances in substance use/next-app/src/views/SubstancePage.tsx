...
const SubstancePage = () => {
  const { slug, trackerId, toolId, contentId, substep, step } = useAppParams();
  ...
  const setActiveTracker = (id: string | null) => {
    if (id) router.replace(`/${slug}/tracker/${id}`);
    else router.replace(`/${slug}`);
  };

  const setActiveTool = (id: string | null) => {
    if (id) router.replace(`/${slug}/tool/${id}`);
    else router.replace(`/${slug}`);
  };
  ...
  const executeReset = async () => {
    ...
    router.replace(`/${slug}/onboarding/0`);
    setOnboarded(false);
  ...
  if (!onboarded) {
    return (
      <SubstanceOnboarding
        ...
        onStepChange={(step) => {
          router.push(`/${slug}/onboarding/${step}`);
        }}
