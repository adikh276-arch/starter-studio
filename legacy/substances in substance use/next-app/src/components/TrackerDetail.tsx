    ...
    for (const field of tracker.fields) {
      ...
      if (field.type === 'slider' && val === 0 && (field.key.toLowerCase().includes('units') || field.key.toLowerCase().includes('amount') || field.key.toLowerCase().includes('spent'))) {
         ...
         if (drankKey && values[drankKey] === 'Yes') {
            toast.error(`${t(`quit.substances.${substance.slug}.trackers.${tracker.id}.fields.${field.key}.label`, field.label) as string} cannot be 0 if you used today.`);
            isValid = false;
            break;
         }
      }
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-background/95 backdrop-blur-sm overflow-y-auto">
      <div className="mx-auto max-w-2xl px-5 pb-10">
        ...
        <div className="flex items-center justify-between py-5 sticky top-0 bg-background/90 backdrop-blur-md z-10 border-b border-border/40">
          <div>
            <h2 className="font-display text-xl text-foreground">{t(`quit.substances.${substance.slug}.trackers.${tracker.id}.name`, tracker.name) as string}</h2>
        ...
        >
          ...
          <div className="space-y-5">
            {tracker.fields.map(field => {
              ...
              return (
                <div key={field.key}>
                  <label className="mb-2.5 block text-xs font-bold text-foreground uppercase tracking-wider">{t(`quit.substances.${substance.slug}.trackers.${tracker.id}.fields.${field.key}.label`, field.label) as string}</label>
                  {field.type === 'slider' && (
                      ...
                      {field.options.map(opt => {
                        ...
                        return (
                          <button key={opt} onClick={() => {
                            ...
                            {t(`quit.substances.${substance.slug}.trackers.${tracker.id}.fields.${field.key}.options.${field.options!.indexOf(opt)}`, opt) as string}
                          </button>
                        );
                      })}
                    </div>
