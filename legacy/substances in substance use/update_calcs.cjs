...
for (const slug of Object.keys(replacements)) {
  const oldTextPath = 'scratch/' + slug + '_calc.txt';
  if (fs.existsSync(oldTextPath)) {
    const oldText = fs.readFileSync(oldTextPath, 'utf-8');
    if (content.includes(oldText)) {
      content = content.replace(oldText, replacements[slug]);
