...
const regexes = {
  alcohol: /calculator:\s*\{[\s\S]*?compute:\s*\([\s\S]*?=>\s*\{[\s\S]*?\}(?:,\n\s*note:\s*"[^"]*")?\n\s*\}/,
  tobacco: /calculator:\s*\{[\s\S]*?compute:\s*\([\s\S]*?=>\s*\[[\s\S]*?\];\n\s*\}(?:,\n\s*note:\s*"[^"]*")?\n\s*\}/,
  opioids: /calculator:\s*\{[\s\S]*?compute:\s*\([\s\S]*?=>\s*\[[\s\S]*?\],\n\s*\}/,
  cannabis: /calculator:\s*\{[\s\S]*?compute:\s*\([\s\S]*?=>\s*\{[\s\S]*?\}(?:,\n\s*note:\s*"[^"]*")?\n\s*\}/,
  stimulants: /calculator:\s*\{[\s\S]*?compute:\s*\([\s\S]*?=>\s*\{[\s\S]*?\}(?:,\n\s*note:\s*"[^"]*")?\n\s*\}/,
  benzodiazepines: /calculator:\s*\{[\s\S]*?compute:\s*\([\s\S]*?=>\s*\{[\s\S]*?\}(?:,\n\s*note:\s*"[^"]*")?\n\s*\}/,
  kratom: /calculator:\s*\{[\s\S]*?compute:\s*\([\s\S]*?=>\s*\{[\s\S]*?\}(?:,\n\s*note:\s*"[^"]*")?\n\s*\}/,
  mdma: /calculator:\s*\{[\s\S]*?compute:\s*\([\s\S]*?=>\s*\{[\s\S]*?\}(?:,\n\s*note:\s*"[^"]*")?\n\s*\}/
};

for (const slug of Object.keys(replacements)) {
  const match = content.match(regexes[slug]);
  if (match) {
    // Preserve any trailing comma if we matched exactly up to the brace
    content = content.replace(match[0], replacements[slug]);
    console.log('Replaced ' + slug);
  } else {
    console.log('FAILED to find ' + slug + ' exact match');
  }
}
