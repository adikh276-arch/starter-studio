...
import '@/features/emotional/lib/i18n/i18n';
import { loadGlobalResource as loadLocale } from '@/features/emotional/lib/i18n/i18n';
...
export function GuidedActivityClient({ concern, activityName }: GuidedActivityClientProps) {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const res = await mockFetch(apiPath(`/api/guided-series/history?concern=${encodeURIComponent(concern)}&activity=${encodeURIComponent(activityName)}`));
      if (res.ok) {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const res = await mockFetch(apiPath('/api/guided-series/save'), {
        method: 'POST',
