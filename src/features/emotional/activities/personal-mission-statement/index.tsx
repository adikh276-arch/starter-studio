'use client';
import { parseDbDate } from '@/features/emotional/lib/dateUtils';
...
import { PremiumLayout } from '@/features/emotional/components/shared/PremiumLayout';
import { PremiumIntro } from '@/features/emotional/components/shared/PremiumIntro';
import { PremiumComplete } from '@/features/emotional/components/shared/PremiumComplete';
import ShareModal from '@/features/emotional/components/shared/ShareModal';
import { apiPath, mockFetch } from "@/features/emotional/lib/apiPath";
...
function PersonalMissionStatementInner() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const res = await mockFetch(apiPath('/api/personal-mission-statement'));
      if (res.ok) {
  ...
  const handleDeleteStatement = async (id: string) => {
    try {
      const res = await mockFetch(apiPath(`/api/personal-mission-statement?id=${id}`), {
        method: 'DELETE',
      });
