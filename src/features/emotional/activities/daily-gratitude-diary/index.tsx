...
function DailyGratitudeInner() {
  ...
  const fetchPastEntries = useCallback(async () => {
    setLoading(true);
    try {
      const res = await mockFetch(apiPath('/api/daily-gratitude-diary'));
      if (res.ok) {
  ...
  const saveEntry = async () => {
    ...
    try {
      await mockFetch(apiPath('/api/daily-gratitude-diary'), {
        method: 'POST',
  ...
  const deleteEntry = async (id: string) => {
    try {
      const res = await mockFetch(apiPath(`/api/daily-gratitude-diary?id=${id}`), {
        method: 'DELETE',
      });
      if (res.ok) {
