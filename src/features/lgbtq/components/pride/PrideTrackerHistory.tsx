/**
 * Ported from
 * `legacy/LGBTQ+ Selfcare/src/features/pride/components/PrideTrackerHistory.tsx`.
 * Difference: reads history from the local tracker store instead of a
 * server action.
 */
...
import { getTrackerHistory, type TrackerEntry } from "../../lib/trackerStore";
...
export const PrideTrackerHistory: React.FC<PrideTrackerHistoryProps> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await getTrackerHistory(tableName, "local");
  ...
  if (entries.length === 0) {
    return (
      <div className="premium-card p-12 text-center space-y-4">
        ...
        <p className="text-lg text-muted-foreground font-medium italic">{displayEmptyMessage}</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-10">
      {entries.map((entry, idx) => (
        ...
        >
          ...
          <div className="p-6">{renderEntry(entry)}</div>
        </motion.div>
      ))}
    </div>
  );
