...
const Index = () => {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getDynamicMiniHistory('pride_journal_entries', userId);
      const rows = res.success && res.data ? res.data : [];
      const dbEntries = rows.map((r: any) => r.data as SavedJournal);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await saveDynamicMiniEntry('pride_journal_entries', userId, journal);
  ...
  const handleBack = () => {
    if (screen === "history") {
      ...
      navigate("/lgbtq" + window.location.search);
    }
  };
  ...
  const renderScreen = () => {
    switch (screen) {
      ...
      case "completion":
        return <CompletionScreen entries={entries} onSave={handleSave} onRestart={handleRestart} onViewHistory={handleViewHistory} onBackToHub={() => navigate("/lgbtq" + window.location.search)} />;
      case "history":
