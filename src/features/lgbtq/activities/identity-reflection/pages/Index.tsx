...
import { saveDynamicMiniEntry, getDynamicMiniHistory, deleteDynamicMiniEntry } from "@/features/lgbtq/lib/dynamicMinisShim";
...
const Index = () => {
  ...
  const fetchConstellations = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const res = await getDynamicMiniHistory('identity_reflection_entries', userId);
  ...
  const handleDelete = useCallback(async (id: string) => {
    ...
    try {
      if (typeof id === 'number' || id.length < 30) {
         await deleteDynamicMiniEntry('identity_reflection_entries', id, userId);
      } else {
         const local = JSON.parse(localStorage.getItem("constellations") || "[]");
