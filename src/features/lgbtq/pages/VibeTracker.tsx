/**
 * Ported from `legacy/LGBTQ+ Selfcare/src/features/pride/trackers/VibeTracker.tsx`.
 * Faithful UI port; persistence moved to `lib/trackerStore`.
 */
import { useState } from "react";
import { Check, Zap, Flame, Cloud, Wind } from "lucide-react";
import { useTranslation } from "@/features/coaching/lib/i18n";
import { saveGenericTrackerEntry, triggerActivityWebhook } from "../lib/trackerStore";
...
import { PrideSuccessState } from "../components/pride/PrideSuccessState";
import { ShareModal } from "../components/pride/ShareModal";
import { PrideTrackerHistory } from "../components/pride/PrideTrackerHistory";
...
export default function VibeTracker() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await saveGenericTrackerEntry("vibe_entries", "local", {
        vibe: selectedVibe,
    ...
    <div className="activity-root py-12 md:py-20">
      ...
      <div className="activity-container-sm">
        <PrideActivityHeader
          ...
          subtitle={
            showHistory
              ? t("Look back at your energy journey")
              : t("Tune in to your internal frequency.")
          }
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-xl bg-gradient-to-br ${
                      VIBES.find((v) => v.id === entry.vibe)?.color || "from-gray-400 to-gray-600"
                    } text-white`}
                  >
                ...
                {entry.reflections && entry.reflections.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {entry.reflections.map((r: string) => (
                      <span
                        key={r}
                        className="px-3 py-1 rounded-full bg-pride-purple/10 text-pride-purple text-xs font-bold"
                      >
                        {r}
                      </span>
                    ))}
                ...
                {VIBES.map((v) => (
                  ...
                  >
                    ...
                    <div className="relative h-full flex flex-col items-center justify-center gap-3 text-white">
                      <div
                        className={`p-4 rounded-3xl bg-white/20 backdrop-blur-md border border-white/30 transition-transform duration-500 ${
                          selectedVibe === v.id ? "scale-110 rotate-3" : "group-hover:rotate-6"
                        }`}
                      >
                        {v.icon}
                      </div>
                    ...
                    <button
                      ...
                      className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all duration-300 ${
                        isSelected
                          ...
                          : "border-black/5 bg-gray-50/50 text-gray-500 hover:border-pride-purple/30 hover:bg-white"
                      }`}
                    >
                      ...
                      <div
                        className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${
                          isSelected
                            ? "bg-pride-purple border-pride-purple"
