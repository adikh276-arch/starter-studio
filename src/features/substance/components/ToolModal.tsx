...
const ToolModal = ({ toolId, substance, onClose }: Props) => {
  ...
  const setActiveActivity = (id: string | null) => {
    if (id) navigate(`/substance/${slug}/tool/activities/${id}`, { replace: true });
    else navigate(`/substance/${slug}/tool/activities`, { replace: true });
  };

  const setActiveSubstep = (step: string | null) => {
    if (step) navigate(`/substance/${slug}/tool/${toolId}/${contentId}/${step}`, { replace: true });
    else navigate(`/substance/${slug}/tool/${toolId}/${contentId}`, { replace: true });
  };

  const setActiveArticle = (id: string | null) => {
    if (id) navigate(`/substance/${slug}/tool/learn/${id}`, { replace: true });
    else navigate(`/substance/${slug}/tool/learn`, { replace: true });
  };
    ...
    if (step < 10) setStep(step + 1);
      ...
      if (substance.slug === 'alcohol') {
        try {
          ...
          if (entryId) {
            console.log("Mocking webhook success for assessment");
          } else {
