...
const SubstancePage = () => {
  ...
  const setActiveTracker = (id: string | null) => {
    if (id) navigate(`/substance/${slug}/tracker/${id}`, { replace: true });
    else navigate(`/substance/${slug}`, { replace: true });
  };

  const setActiveTool = (id: string | null) => {
    if (id) navigate(`/substance/${slug}/tool/${id}`, { replace: true });
    else navigate(`/substance/${slug}`, { replace: true });
  };
  ...
  const executeReset = async () => {
    ...
    navigate(`/substance/${slug}/onboarding/0`, { replace: true });
    setOnboarded(false);
