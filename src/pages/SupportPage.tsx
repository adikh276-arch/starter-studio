      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-[#F1F7F7] flex items-center justify-center">
              <Ticket size={16} className="text-[#0B2545]" />
            </div>
        ...
        {submitted ? (
          <div className="flex flex-col items-center py-10 px-6 text-center">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 260, damping: 20 }}
              className="w-16 h-16 rounded-full bg-[#F1F7F7] flex items-center justify-center mb-4"
            >
              <CheckCircle2 size={32} className="text-[#13B5B1]" />
            </motion.div>
            ...
            <div className="flex items-center gap-1.5 mt-4 bg-[#F1F7F7] rounded-xl px-4 py-2">
              <Clock size={13} className="text-[#0B2545]" />
              <span className="text-[#0B2545] text-xs">Ticket #MC-{Math.floor(100000 + Math.random() * 900000)}</span>
            </div>
            <button onClick={onClose} className="mt-6 px-6 py-2.5 bg-[#0B2545] hover:bg-[#13B5B1] text-white rounded-xl text-sm transition-colors">Done</button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
                  ...
                  >
                    {CATS.map(c => (
                      <button key={c} type="button" onClick={() => { setCategory(c); setCatOpen(false); }}
                        className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${c === category ? "bg-[#F1F7F7] text-[#0B2545] font-medium" : "text-[#64748B] hover:bg-[#F8FAFC]"}`}
                      >{c}</button>
                    ))}
                  </motion.div>
                )}
            ...
            <div>
              ...
              <input type="text" value={subject} onChange={e => setSubject(e.target.value)} placeholder="Brief summary of your issue"
                className="w-full border border-[#E5EAF0] rounded-xl px-3.5 py-2.5 text-sm text-[#0F172A] placeholder-[#E5EAF0] bg-[#F8FAFC] focus:outline-none focus:ring-2 focus:ring-[#F1F7F7] focus:border-[#0B2545] transition-all"
              />
            </div>
            <div>
              ...
              <textarea rows={4} value={desc} onChange={e => setDesc(e.target.value)} placeholder="Describe your issue in detail..."
                className="w-full border border-[#E5EAF0] rounded-xl px-3.5 py-2.5 text-sm text-[#0F172A] placeholder-[#E5EAF0] bg-[#F8FAFC] resize-none focus:outline-none focus:ring-2 focus:ring-[#F1F7F7] focus:border-[#0B2545] transition-all"
              />
            </div>
            <motion.button type="submit" disabled={!subject.trim() || !desc.trim() || loading}
              ...
              className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-medium transition-all ${
                subject.trim() && desc.trim() ? "bg-[#0B2545] hover:bg-[#13B5B1] text-white shadow-sm" : "bg-[#F8FAFC] text-[#E5EAF0] cursor-not-allowed"
              }`}
            >
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-end justify-end p-4 md:p-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      <motion.div
        ...
        style={{ height: 480 }}
      >
        <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-[#0B2545] to-[#13B5B1]">
          <div className="flex items-center gap-2.5">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50">
          {messages.map((m, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className={`flex ${m.from === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[80%] px-3.5 py-2.5 rounded-2xl text-xs leading-relaxed ${m.from === "user" ? "bg-[#0B2545] text-white rounded-br-sm" : "bg-white border border-[#E5EAF0] text-[#64748B] rounded-bl-sm shadow-sm"}`}>{m.text}</div>
            </motion.div>
          ))}
          {thinking && (
        ...
        <div className="p-3 bg-white border-t border-slate-100 flex gap-2">
          <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="Type a message..."
            className="flex-1 border border-[#E5EAF0] rounded-xl px-3 py-2 text-sm text-[#0F172A] placeholder-[#E5EAF0] bg-[#F8FAFC] focus:outline-none focus:ring-2 focus:ring-[#F1F7F7] focus:border-[#0B2545] transition-all"
          />
          <motion.button whileTap={{ scale: 0.9 }} onClick={send} disabled={!input.trim()}
            className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${input.trim() ? "bg-[#0B2545] hover:bg-[#13B5B1] text-white" : "bg-[#F8FAFC] text-[#E5EAF0]"}`}
...
export function SupportPage() {
  ...
  const cards = [
    ...
    {
      icon: Building2, iconBg: "bg-[#F1F7F7]", iconColor: "text-[#0B2545]",
        ...
        <main className="max-w-[1000px] w-full mx-auto px-4 md:px-6 py-4 md:py-10 pt-[72px] md:pt-10">
          ...
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="flex items-center gap-3 mb-8">
            <div className="w-10 h-10 bg-[#F1F4F8] rounded-md flex items-center justify-center flex-shrink-0">
              <HelpCircle size={20} className="text-[#0F172A]" strokeWidth={2} />
            </div>
            <div>
